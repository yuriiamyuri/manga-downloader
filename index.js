...
async function downloadAndCompressImage(url, filepath) {
  ...
  await sharp(buffer)
    .resize({ width: 600 }) // Further reduce image dimensions
    .png({ quality: 50, compressionLevel: 9, effort: 9 }) // Increase compression level and lower quality
    .toFile(tempFilePath);
...
async function createPDFWithImages(imageUrls, outputPath) {
  ...
  for (let i = 0; i < imageUrls.length; i++) {
    ...
    try {
      const downloadedImagePath = await downloadAndCompressImage(
        url,
        tempFilePath
      );
